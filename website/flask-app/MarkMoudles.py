...
def mark_add():
    ...
    mark=Bookmark(CatalogNumber=int(catalog_number),BookmarkDescription=description)
    mark.user=g.user
...
def mark_remove():
    ...
    mark=Bookmark.query.filter_by(UserID=int(g.user.UserID),BookID=int(book_id),CatalogNumber=int(catalog_number)).first()
    if mark is None:
